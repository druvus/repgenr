...
def test_native_scaling_skips_chunking(workdir: Path, genome_files, fake_tool) -> None:
    ctx = WorkdirContext(workdir, create=True)
    # fake tool supports native scaling, so process_size is ignored
    result = run(ctx, DereplicateParams(tool="fake", process_size=1))
    assert len(result.representatives) == 1
